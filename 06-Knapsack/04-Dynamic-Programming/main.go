...
func dynamicProgramming(items []Item, allowedWeight int) ([]Item, int, int) {
	...
	for i := 1; i < numItems; i++ {
		for w := 0; w <= allowedWeight; w++ {
			valueIfNot := solutionValue[i-1][w]
			valueIfYes := 0
			if items[i].weight <= w {
				valueIfYes = items[i].value + solutionValue[i-1][w-items[i].weight]
			}
			// fmt.Printf("  -- i=%d w=%d valueIfNot=%d valueIfYes=%d\n",
			//	i, w, valueIfNot, valueIfYes)
			if valueIfNot > valueIfYes {
				solutionValue[i][w] = valueIfNot
				prevWeight[i][w] = w
			} else {
				solutionValue[i][w] = valueIfYes
